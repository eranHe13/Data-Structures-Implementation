...
public class Graph<V extends Comparable<V>, E> implements IGraph<V, E>{
    ...
    public String toStringExtended() {
        ...
        for(V v: graph.keySet()){
            sb.append( v.toString() + ":" );
            List<Edge> TMP =    graph.get(v);
            for (Edge e : TMP){
                sb.append("{");
                sb.append(e.getV1() + "," + e.getV2() + "}" );
                if(e.getEdgeLabel() instanceof  Number || e.getEdgeLabel() instanceof Weighted) {
                    sb.append("(" + e.getEdgeLabel() + "),");
                }
    ...
    private class Edge{
        ...
        public Edge(V v1 , V v2 ,E edge_L){
            this.v1 = v1;
            this.v2 = v2;
            this.edgeLabel = edge_L;
        }
